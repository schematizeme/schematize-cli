...
const LEITURA_SEGURA: &[&str] = &[
    "ls", "cat", "head", "tail", "wc", "stat", "file", "du", "df", "free", "uptime", "date",
    "hostname", "whoami", "id", "uname", "pwd", "ps", "grep", "egrep", "zgrep", "journalctl",
    "systemctl", "docker", "podman", "git", "ip", "ss", "netstat", "dig", "nslookup", "curl",
    "echo", "which", "env", "lsblk", "mount", "nproc", "lscpu", "top", "htop", "find", "tree",
];
...
fn catastrofico_por_estrutura(c: &Comando) -> Option<&'static str> {
    ...
    if c.operandos.iter().any(|o| e_dispositivo_de_bloco(o)) {
        ...
        let escreve = c.binario == "dd" && c.operandos.iter().any(|o| o.starts_with("of=") && e_dispositivo_de_bloco(o));
        let escreve = escreve || matches!(c.binario.as_str(), "mkfs" | "wipefs" | "blkdiscard" | "tee" | "cp" | "mv");
        if escreve {
...
fn descrever_nao_ascii(cmd: &str) -> String {
...
mod tests {
    ...
    fn unicode_hostil_nunca_passa_calado_nem_no_modo_livre() {
        ...
        for (cmd, rotulo) in [
            ...
            match avaliar(&livre, cmd) {
                Veredito::Confirm(m) => assert!(m.contains("U+"), "{rotulo}: tem que dizer QUAL: {m}"),
    ...
    fn mesmo_comando_muda_de_veredito_conforme_o_modo() {
        ...
        assert!(
            matches!(avaliar(&perfil(ModoPolitica::ReadOnly, Ambiente::Hml), cmd), Veredito::Deny(_)),
            "readonly recusa o que escreve"
        );
        assert!(
            matches!(avaliar(&perfil(ModoPolitica::OpsVerbs, Ambiente::Hml), cmd), Veredito::Deny(_)),
    ...
    fn readonly_deixa_ler_e_recusa_escrever_no_mesmo_binario() {
        ...
        assert_eq!(avaliar(&p, "/bin/ls -la /var/log"), Veredito::Allow, "caminho absoluto conta pelo binário");
        for escreve in ["systemctl stop app", "docker rm app", "git push", "docker run x", "curl -o /tmp/x http://y"] {
            assert!(matches!(avaliar(&p, escreve), Veredito::Deny(_)), "{escreve:?} escreve");
        }
        assert!(matches!(avaliar(&p, "apt install nginx"), Veredito::Deny(_)), "binário fora da lista");
    }
    ...
    fn prd_sempre_pede_confirmacao_ate_pro_comando_mais_inofensivo() {
        ...
        match avaliar(&p, "uptime") {
            Veredito::Confirm(m) => assert!(m.contains("PRODUÇÃO"), "o motivo tem que dizer por quê: {m}"),
    ...
    fn o_gate_de_prd_endurece_mas_nunca_afrouxa() {
        ...
        assert!(matches!(avaliar(&perfil(ModoPolitica::ReadOnly, Ambiente::Prd), "apt install x"), Veredito::Deny(_)));
    }
    ...
    fn ops_verbs_aceita_o_verbo_e_recusa_o_resto() {
        ...
        match avaliar_com_catalogo(&p, "restart", &cat()) {
            Veredito::Deny(m) => {
                assert!(m.contains("deploy") && m.contains("status"), "tem que listar os verbos: {m}");
    ...
    fn catalogo_vazio_recusa_e_ensina_em_vez_de_virar_modo_livre() {
        ...
        match avaliar_com_catalogo(&p, "deploy", &[]) {
            Veredito::Deny(m) => assert!(m.contains("--seed") || m.contains("--add"), "tem que ensinar: {m}"),
    ...
    fn nao_existe_valvula_de_escape_no_modulo() {
        ...
        for proibido in ["skip_policy", "force_policy", "bypass_policy", "ignore_policy"] {
            assert!(!producao.contains(proibido), "válvula de escape {proibido:?} não pode existir");
        }
    }
